...
@Autonomous(name="Depot Auto Without Camera", group="Testy")
public class depot01 extends LinearOpMode{
    ...
    public void runOpMode(){
        ...
        offHook();
        doublePID(-4.17, 2.51, 1.49, false);
        armExtend(slideMax, 1);
        dropMarker();
        armExtend(0, 1);
        //.781
        for(int i = 0; i < 3 && cheese == "none"; i++){
            if(i == 0){
                doublePID(-4.17, 2.51, 1.7, false); //Center
                cheese = "center";
            }
            if(i == 1){
                doublePID(-4.28, 2.74, 0.87, false); //Right
            }
            if(i == 2){
                doublePID(-4.15, 2.68, 2.25, false); //Left (Closest to enemy crater)
                cheese = "left";
            }
            //checkCheese();
        }
        ...
        if(cheese == "right"){
            goAngle = 0.65;
            doublePID(-4.28, 2.74, goAngle, false);
            setChungoids(.75);
            armExtend(-963, 1);
        ...
        if(cheese == "center"){
            goAngle = 1.7;
            doublePID(-4.17, 2.51, goAngle, false);
            armExtend(-200, 1);
            setChungoids(.8);
            shell.intake1.setPower(-1);
            shell.intake2.setPower(1);
        ...
        if(cheese == "left"){
            goAngle = 1.96;
            doublePID(-3.15, 2.68, goAngle, false);
            setChungoids(.75);
            armExtend(-963, 1);
            shell.intake1.setPower(-1);
            shell.intake2.setPower(1);
        ...
        setChungoids(.58);
        armExtend(0, 1);
        //doublePID(-7.74, 0.16, goAngle, true);
        doublePID(-7.74, 0.16, 3.21, false);
        doublePID(-8.36, -7.46, 3.21, false);
        doublePID(-12.67, -14.97, 3.74, false);
        driveTime(1000, 1, 1);
        armExtend(-3241, 1);
        setChungoids(0.84);
